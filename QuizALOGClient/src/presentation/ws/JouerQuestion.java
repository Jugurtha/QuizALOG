...
public class JouerQuestion extends JFrame {
	...
	public JouerQuestion(wsclient.Game game) {
	...
	private void chargerUneQuestion() {
	 ...
	 // évaluer la réponse du joueur: si vrai incrémenter le score sinon afficher un message d'erreur et passer à la suivante
		private void evaluerReponse() {
			...
			gq.setGame(game);
			gq.setAnswer(currentQuestion.getAnswers()[repList.getSelectedIndex()]);
			gameQuestions.add(gq);
			
			if (checkAnswer(currentQuestion, currentQuestion.getAnswers()[repList.getSelectedIndex()])){   // cas de réponse juste
	...
	private void loadQuestions() {
		...
		try {
			questions = quizWs.loadQustions(4);
			/*
...
//System.out.println(question);
	...
	private Question nextQuestion()
	{
		...
		if(indexCurrentQuestion>=questions.length)
			return null;
		return questions[indexCurrentQuestion];	
	}
	...
	private void saveGame()
	{
		...
		try {
			quizWs.saveGame(game, gameQuestions.toArray(new GameQuestion[1]));			
		} catch (RemoteException e) {
			e.printStackTrace();
		}
